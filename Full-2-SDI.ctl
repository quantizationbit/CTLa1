...
)
{
  ...
  float RGBSDI[3];
  RGBSDI[0] = CV_BLACK_SDI + (CV_WHITE_SDI - CV_BLACK_SDI) * RGBFULL[0];
  RGBSDI[1] = CV_BLACK_SDI + (CV_WHITE_SDI - CV_BLACK_SDI) * RGBFULL[1];
  RGBSDI[2] = CV_BLACK_SDI + (CV_WHITE_SDI - CV_BLACK_SDI) * RGBFULL[2]; 
  



  // This step converts integer CV back into 0-1 which is what CTL expects
  RGBSDI = mult_f_f3( 1./(pow(2,BITDEPTH)-1), RGBSDI);
